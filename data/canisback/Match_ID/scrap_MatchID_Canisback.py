...
url = "https://canisback.com/matchId/"
...
for index, i in enumerate(soup.findAll("a")[1:]):
    remoteFiles.append(str(i["href"]).replace("json", "") + update[index])
localFiles = []
for index, i in enumerate(os.listdir(path = "./data/metadata/canisback/Match_ID/")):
...
for index_i, i in enumerate(remoteFiles):
    ...
    if(len(localFiles) == 0): # Used for the first download;
        urllib.request.urlretrieve(url      = ("https://canisback.com/matchId/" + str(i).split(".")[0] + ".json"), 
                                   filename = ("./data/metadata/canisback/Match_ID/" + remoteFiles[index_i]))
        continue
    else:
        for j in localFiles:   
            counter += 1
            if(str(i).split(".")[0] == str(j).split(".")[0]):
                if(str(i).split(".")[1] == str(j).split(".")[1]):
                    ...
                    urllib.request.urlretrieve(url      = ("https://canisback.com/matchId/" + str(i).split(".")[0] + ".json"), 
                                               filename = ("./data/metadata/canisback/Match_ID/" + remoteFiles[index_i]))
                    os.remove(path = "./data/metadata/canisback/Match_ID/" + str(j))
                    break
            else:
